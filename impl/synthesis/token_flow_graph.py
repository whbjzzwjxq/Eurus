...
from impl.dsl.dsl import AFLAction, Sketch
from impl.dsl.financial_constraints import extract_rw_vars
    ...
    def mutation(self, sketch: Sketch):
        ...
        for i, cur_a in enumerate(actions):
            ...
            if cur_a.action_name == "swap":
                ...
                read_vars, _ = extract_rw_vars(cur_a._constraints)
                # The balance of action doesn't influence the price.
                ...
                for f in self.func_summarys:
                    ...
                    _, write_vars = extract_rw_vars(f._constraints)
