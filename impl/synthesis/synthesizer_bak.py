...
from impl.toolkit.const import ENV, MODIFIER
    ...
    def infer_candidates(self):
        ...
        for func in self.test_ctrt.functions:
            ...
            if not is_foray_action:
                continue
            func_summary = self._infer_action_token_flows(func)
            self.func_summarys[func_summary.func_sig] = func_summary

        # Generate sketch
        tokens = [VOID] + self.config.assets
        accounts = [ATTACKER, DEAD, ENV] + list(self.role2ctrt.keys())
        ...
        return candidates[: gt_idx + 1], timecost

    def _infer_action_token_flows(self, func: SliFunction) -> AFLAction:
        action = self.map_func_to_action(func)
        token_flows = self.infer_token_flows(action)
        action.update(token_flows)
        return action
